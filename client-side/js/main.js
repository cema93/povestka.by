(function() {
  ...
  function setConfigHTML(data) {
    ...
    document.getElementsByClassName("mainImage")[0].src = '/doika/images/' + data.innerText.titleImage;
  ...
  function submitbutton() {
    ...
    if(!sum) {
      document.getElementsByClassName("module-donate__warning")[0].innerHTML = "Сумма не выбрана или не введена!";
      showAlert();
    } else if ( sum > dataConfig.maxDonateAmount) {
      document.getElementsByClassName("module-donate__warning")[0].innerHTML = 'Сумма дара не может быть больше '+ dataConfig.maxDonateAmount +' руб!';
      showAlert();
    } else if ( sum < dataConfig.minDonateAmount) {
      document.getElementsByClassName("module-donate__warning")[0].innerHTML = 'Сумма дара не может быть меньше '+ dataConfig.minDonateAmount +' руб!';
      showAlert();
    } else {
