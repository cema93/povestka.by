...
class CampaignPageClient extends Model
{
  ...
  public static function getCampaignClientArray($id)
  {
    ...
    $arrCampaign = [
      ...
      "innerText" => [
        ...
        "resultsText" => 'Собрано <span class="summ__highlight">'.self::getCurrentFunds($id).'</span> из <span class="goal__highlight">'.$campaignConfigurations->required_amount.'</span> BYN', //текст, показывающий, сколько собрано, разметка упростит стилизацию
  ...
  public static function getShortCampaignClientArray($id)
  {
    $campaign = Campaign::find($id);
    if(!$campaign){
      return "Campaign not find!";
